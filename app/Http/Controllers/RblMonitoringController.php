...
class RblMonitoringController extends Controller
{
    ...
    public function events(Request $request)
    {
        ...
        $events = RblEvent::with(['target', 'list'])->where('first_seen_at', '<', $until)
        ...
        return view('rbl.events', [
            'events' => $events, 'targets' => RblTarget::orderBy('name')->get(),
    ...
    public function reports(Request $request)
    {
        ...
        $summary = [
            ...
            'Eventos abertos no período' => RblEvent::where('first_seen_at', '>=', $start)->where('first_seen_at', '<', $until)->count(),
            'Eventos resolvidos no período' => RblEvent::where('resolved_at', '>=', $start)->where('resolved_at', '<', $until)->count(),
        ];
        if ($request->input('format') === 'csv') {
            $filename = 'rbl-report-'.$start->toDateString().'-to-'.$until->subDay()->toDateString().'.csv';

            return response()->streamDownload(function () use ($checks, $summary) {
                ...
                $row([]);
                $row(['Alvo', 'Valor verificado', 'RBL', 'Status', 'Data', 'Resposta', 'Erro']);
                foreach ((clone $checks)->with(['target', 'list'])->lazyById(500) as $check) {
                    $row([$check->target?->name, $check->checked_value, $check->list?->name, $check->status,
        ...
        return view('rbl.reports', compact('summary', 'topTargets', 'topLists'));
    }
}
