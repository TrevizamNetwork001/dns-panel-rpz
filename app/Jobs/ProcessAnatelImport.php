...
class ProcessAnatelImport implements ShouldQueue {
 ...
 public function handle(AnatelPdfExtractor $extractor,AnatelImporter $importer):void{
  ...
  try{$payload=$extractor->extract([Storage::disk('anatel')->path($import->storage_path)]);$import->update(['progress'=>70]);$importer->apply($import->lista,$import,$payload);$import->update(['progress'=>100]);$this->audit($import,'anatel.import.completed',"Importação ANATEL concluída: {$import->fresh()->new_count} novos");}
