...
class AnatelExclusionMatcher
{
    ...
    public function matches(string $domain, iterable $exclusions): bool
    {
        foreach ($exclusions as $exclusion) {
            ...
            if ($exclusion->type === 'exact' && hash_equals($exclusion->value, $domain)) {
                return true;
            }
            if ($exclusion->type === 'regex' && $this->isValidRegex($exclusion->value) && preg_match($this->pattern($exclusion->value), $domain) === 1) {
                return true;
            }
        }
