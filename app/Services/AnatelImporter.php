...
class AnatelImporter
{
    ...
    {
        if (! $lista->isAnatel()) throw new RuntimeException('A lista informada não é ANATEL.');
        $file=$payload['files'][0]??[];$raw=array_values(array_unique(array_filter($payload['domains']??[],'is_string')));$normalized=[];$invalid=0;
        foreach($raw as$candidate){$domain=$this->normalizer->normalize($candidate);if($domain===null){$invalid++;continue;}$normalized[$domain]=true;}
        $domains=array_keys($normalized);$candidates=(int)($file['candidates']??count($raw));
        if($domains===[]||($candidates>=100&&count($domains)<($candidates*(float)config('anatel.blocked_ratio')))){$import->update(['pages'=>(int)($file['pages']??0),'candidates_count'=>$candidates,'valid_count'=>count($domains),'invalid_count'=>$invalid+(int)($file['invalid']??0),'status'=>'blocked','progress'=>100,'error'=>'Extração vazia ou incoerente; nenhuma alteração aplicada.','finished_at'=>now()]);throw new RuntimeException('Importação bloqueada.');}
        $exclusions=$lista->anatelExclusions()->where('active',true)->get();$new=0;$existing=0;$reactivated=0;$excluded=0;$now=now();
        DB::transaction(function()use($lista,$import,$domains,$exclusions,$now,&$new,&$existing,&$reactivated,&$excluded){
            DB::table('anatel_import_domains')->where('anatel_import_id',$import->id)->delete();
            foreach(array_chunk($domains,400)as$chunk){$current=DB::table('dominios')->where('lista_id',$lista->id)->whereIn('dominio',$chunk)->get()->keyBy('dominio');$rows=[];foreach($chunk as$domain){$record=$current->get($domain);if($this->matcher->matches($domain,$exclusions)){$result='excluded';$excluded++;}elseif(!$record){$result='new';$new++;}elseif(!$record->ativo&&$record->inactive_reason!=='anatel_exclusion'){$result='reactivated';$reactivated++;}else{$result='existing';$existing++;}$rows[]=['anatel_import_id'=>$import->id,'domain'=>$domain,'result'=>$result,'created_at'=>$now,'updated_at'=>$now];}DB::table('anatel_import_domains')->insert($rows);}
        });
    ...
    {
        if($import->status!=='awaiting_approval')throw new RuntimeException('Importação não está aguardando aprovação.');
