...
return new class extends Migration
{
    public function up(): void
    {
        Schema::create('anatel_imports', function (Blueprint $table) {
            ...
            $table->unsignedInteger('unblocked_count')->default(0);
            $table->string('status')->default('pending');
            $table->text('error')->nullable();
            $table->timestamp('started_at')->nullable();
            $table->timestamp('finished_at')->nullable();
            $table->timestamps();
            $table->index(['lista_id', 'created_at']);
            $table->index('status');
        });
        Schema::create('anatel_import_domains', function (Blueprint $table) {
            $table->id();
            $table->foreignId('anatel_import_id')->constrained('anatel_imports')->cascadeOnDelete();
            $table->string('domain');
            $table->string('result');
            $table->timestamps();
            $table->unique(['anatel_import_id', 'domain']);
            $table->index(['anatel_import_id', 'result']);
        });
    }

    public function down(): void
    {
        Schema::dropIfExists('anatel_import_domains');
        Schema::dropIfExists('anatel_imports');
    }
};
