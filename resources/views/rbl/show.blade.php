...
<div class="panel"><h2>Dados do alvo</h2><p>Grupo: {{ $target->group?->name ?? '—' }}</p><a class="button" href="{{ route('rbl.targets.edit', $target) }}">Editar alvo</a><p>{{ $target->type }}: {{ $target->value }} · Categoria: {{ $target->category ?? '—' }} · {{ $target->enabled ? 'Ativo' : 'Desativado' }}</p><p>{{ $target->description }}</p><p>Resultado da última verificação: <strong>{{ $target->last_status ?? 'unchecked' }}</strong> · {{ $target->last_checked_at?->format('d/m/Y H:i:s') ?? 'Nunca' }}</p>
...
<ul>@forelse($listedChecks as $check)<li>{{ $check->list->name }} — {{ $check->checked_value }} — {{ $check->response }} — {{ $check->checked_at->format('d/m/Y H:i:s') }}</li>@empty<li>Nenhuma.</li>@endforelse</ul></div>
...
@forelse($events as $event)<tr><td>{{ $event->list->name }}</td><td>{{ $event->status }}</td><td>{{ $event->first_seen_at->format('d/m/Y H:i:s') }}</td><td>{{ $event->last_seen_at->format('d/m/Y H:i:s') }}</td><td>{{ $event->resolved_at?->format('d/m/Y H:i:s') ?? '—' }}</td><td>{{ $event->last_response }}<p>IP listado: {{ $event->last_checked_value ?? '—' }}</p><p>{{ $event->notes }}</p></td></tr>@empty<tr><td colspan="6">Nenhum evento registrado.</td></tr>@endforelse
