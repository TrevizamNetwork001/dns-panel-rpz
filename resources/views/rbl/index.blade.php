...
<div class="panel"><h2>Monitoramento por grupo</h2><div class="table-responsive"><table class="data-table"><thead><tr><th>Grupo</th><th>Status</th><th>Alvos</th><th>Listados agora</th><th>Eventos abertos</th></tr></thead><tbody>@forelse($groups as $group)<tr><td><a href="{{ route('rbl.groups.show', $group) }}">{{ $group->name }}</a></td><td>{{ $group->enabled ? 'Ativo' : 'Desativado' }}</td><td>{{ $group->targets_count }}</td><td>{{ $group->listed_count }}</td><td>{{ $group->open_count }}</td></tr>@empty<tr><td colspan="5">Nenhum grupo cadastrado.</td></tr>@endforelse</tbody></table></div></div>
...
<div class="panel"><h2>Alvos</h2><form method="GET">@include('rbl.group-filter')<button class="button">Filtrar alvos</button></form><p>O status reflete a última execução. Eventos permanecem abertos quando uma consulta falha ou é ignorada.</p><div class="table-responsive"><table class="data-table"><thead><tr><th>Nome</th><th>Tipo</th><th>Valor</th><th>Grupo / Categoria</th><th>Status atual</th><th>Progresso CIDR</th><th>Última verificação</th><th>Ações</th></tr></thead><tbody>
@forelse($targets as $target)
<tr><td>{{ $target->name }} @if($target->type === 'cidr' && ($target->category === 'cgnat' || $target->group?->category === 'cgnat'))<span class="status-pill is-info">CGNAT</span>@endif</td><td>{{ strtoupper($target->type) }}</td><td>{{ $target->value }}</td><td>{{ $target->group?->name ?? $target->category ?? '—' }}</td><td><strong>{{ $statusLabels[$target->last_status ?? 'unchecked'] ?? $target->last_status }}</strong>{{ $target->enabled ? '' : ' (desativado)' }}@if($target->type === 'cidr' && $target->last_status === 'partial')<small class="rbl-compact-note">ciclo em andamento</small>@endif</td><td>@if($target->type === 'cidr')@php($state = $target->scanState)<span class="rbl-compact-progress">{{ $state?->scanned_ips ?? 0 }}/{{ $state?->total_ips ?? $target->cidrTotalIps() }} ({{ $state?->progressPercent() ?? 0 }}%) · P {{ $state?->pendingIps() ?? $target->cidrTotalIps() }} · L {{ $state?->listed_ips ?? 0 }} · C {{ $state?->clean_ips ?? 0 }} · E {{ $state?->error_ips ?? 0 }} · ciclo {{ $state?->cycle ?? 0 }}</span>@else — @endif</td><td>{{ $target->last_checked_at?->format('d/m/Y H:i:s') ?? 'Nunca' }}</td><td>
...
@empty<tr><td colspan="8">Nenhum alvo cadastrado.</td></tr>@endforelse
