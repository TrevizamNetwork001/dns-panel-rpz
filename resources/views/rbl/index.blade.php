...
<tr><td>{{ $target->name }}</td><td>{{ $target->type }}</td><td>{{ $target->value }}</td><td>{{ $target->group?->name ?? $target->category ?? '—' }}</td><td>{{ $target->last_status === 'partial' ? 'Parcial' : ($target->last_status ?? 'unchecked') }}{{ $target->enabled ? '' : ' (desativado)' }}</td><td>@if($target->type === 'cidr' && $target->scanState){{ $target->scanState->scanned_ips }}/{{ $target->scanState->total_ips }} ({{ $target->scanState->progressPercent() }}%) · pendentes {{ $target->scanState->pendingIps() }} · listados {{ $target->scanState->listed_ips }} · limpos {{ $target->scanState->clean_ips }} · erros {{ $target->scanState->error_ips }} · ciclo {{ $target->scanState->cycle }}@else — @endif</td><td>{{ $target->last_checked_at?->format('d/m/Y H:i:s') ?? 'Nunca' }}</td><td>
