...
<div class="panel"><h2>Alvos</h2><form method="GET">@include('rbl.group-filter')<button class="button">Filtrar alvos</button></form><p>O status reflete a última execução. Eventos permanecem abertos quando uma consulta falha ou é ignorada.</p><div class="table-responsive"><table class="data-table"><thead><tr><th>Nome</th><th>Tipo</th><th>Valor</th><th>Grupo / Categoria</th><th>Status atual</th><th>Última verificação</th><th>Ações</th></tr></thead><tbody>
@forelse($targets as $target)
<tr><td>{{ $target->name }}</td><td>{{ $target->type }}</td><td>{{ $target->value }}</td><td>{{ $target->group?->name ?? $target->category ?? '—' }}</td><td>{{ $target->last_status ?? 'unchecked' }}{{ $target->enabled ? '' : ' (desativado)' }}</td><td>{{ $target->last_checked_at?->format('d/m/Y H:i:s') ?? 'Nunca' }}</td><td>
